...
export default function OutgoingPage() {
  ...
  useEffect(() => {
    ...
    if (ticketIdParam) {
      ...
      const ticket = tickets.find((t) => t.id === ticketId && t.type === "out");
      if (ticket) {
  ...
  const filtered = useMemo(() => {
    return tickets
      .filter((t) => t.type === "out" && t.site === currentSite)
  ...
  const selected =
    tickets.find((t) => t.id === selectedTicketId && t.type === "out") ||
    filtered[0] ||
    null;
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <BtnPrimary
              onClick={() => router.push("/ticket/out?mode=create")}
              style={{ fontSize: 12 }}
            >
              + Create Ticket
            </BtnPrimary>
            <BtnSecondary
              onClick={() =>
                selected && router.push(`/ticket/out?id=${selected.id}`)
              }
              disabled={!selected}
              style={{ fontSize: 12 }}
            >
        ...
        <div style={{ display: "flex", gap: 16, flex: 1 }}>
          <DataTable
            ...
            maxHeight={420}
            emptyMessage="No outgoing tickets match the current filters."
          />
          ...
          >
            ...
            {selected ? (
              <>
                ...
                <InfoRow
                  label="Loading Loc."
                  value={getLocationName(selected.loadingLocation) || "—"}
                />
