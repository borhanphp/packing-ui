...
export default function PrintOutTicket() {
  ...
  const ticket =
    idStr != null && !Number.isNaN(idNum)
      ? tickets.find((t) => t.id === idNum)
      : null;
  ...
  if (!ticket) {
    return (
      <div style={{ textAlign: "center", padding: 80, color: "#9ca3af", fontFamily: "'Segoe UI', sans-serif" }}>
        ...
        <button onClick={() => router.push("/outgoing")} style={{ marginTop: 16, background: "#3b82f6", color: "#fff", border: "none", borderRadius: 6, padding: "8px 20px", cursor: "pointer", fontSize: 13 }}>Back</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#e8edf2", fontFamily: "'Segoe UI', sans-serif", padding: 32 }}>
      <div className="no-print" style={{ maxWidth: 680, margin: "0 auto 20px", display: "flex", gap: 8, justifyContent: "flex-end" }}>
        ...
        <button onClick={() => router.push("/outgoing")} style={{ background: "#fff", color: "#374151", border: "1px solid #d1d5db", borderRadius: 6, padding: "8px 16px", cursor: "pointer", fontSize: 13 }}>Back</button>
      </div>
        ...
        <div style={{ background: "linear-gradient(135deg, #0f1e3d, #1a3a6b)", padding: "18px 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 11 }}>Weighbridge Ticket — Outgoing</div>
            </div>
          </div>
        ...
        <div style={{ padding: "24px 28px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px 32px" }}>
            ...
            <Section title="Commodity">
              ...
              <PrintRow label="Grade" value={ticket.grade || cmo?.grade || "—"} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px 32px" }}>
            ...
            <Section title="Location">
              <PrintRow label="Loaded From" value={getLocationName(ticket.loadingLocation) || "—"} />
            </Section>
          </div>

          {/* Weights — Out tickets: Tare first, then Gross */}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ background: "#f8fafc" }}>
                  ...
                  {ticket.tareWeights.map((_, i) => (<th key={`t${i}`} style={{ textAlign: "right", padding: "7px 10px", borderBottom: "2px solid #e2e8f0", fontWeight: 600, color: "#64748b", fontSize: 10.5 }}>Entry {i + 1}</th>))}
              ...
              <tbody>
                <tr>
                  <td style={{ padding: "7px 10px", borderBottom: "1px solid #f1f5f9", fontWeight: 600, color: "#374151" }}>Tare (In)</td>
                ...
                <tr>
                  <td style={{ padding: "7px 10px", borderBottom: "1px solid #f1f5f9", fontWeight: 600, color: "#374151" }}>Gross (Out)</td>
                ...
                <tr style={{ background: "#ecfdf5" }}>
                  ...
                  <td colSpan={ticket.tareWeights.length} />
